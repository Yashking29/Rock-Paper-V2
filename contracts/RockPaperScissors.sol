...
contract RPS{
    ...
    enum Move {Null, Rock, Paper, Scissor, Spock, Lizard} // Possible moves. Note that if the parity of the moves is the same the lower one wins, otherwise the higher one. 
    ...
    event Variable(Move c2,uint256 salt,bytes32 hash, address indexed  sender2,address indexed j2);
    ...
    constructor(bytes32 _c1Hash, address _j2) payable {
        ...
        c1Hash=_c1Hash;
        lastAction=now;
        // emit GameCreated(address(this), j1, j2);
    }
     ...
     */
    function solve (Move _c1, uint256 _salt) payable {
        bytes32 a = keccak256(abi.encodePacked(_c1, _salt));
        address sender = msg.sender;
        emit Variable(_c1,_salt,a,sender,j1);
